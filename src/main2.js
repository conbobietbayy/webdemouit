...
// === v2: Pedestrian constants ===
const PEDESTRIAN_COUNT = 20;
const PEDESTRIAN_WALK_SPEED_MIN = 0.62;
const PEDESTRIAN_WALK_SPEED_MAX = 1.18;
const PEDESTRIAN_WALK_PATHS = [
  [new THREE.Vector3(-14, 0, 14), new THREE.Vector3(14, 0, 14)],
  [new THREE.Vector3(14, 0, -13), new THREE.Vector3(-14, 0, -13)],
  [new THREE.Vector3(-14, 0, -13), new THREE.Vector3(-14, 0, 14)],
  [new THREE.Vector3(14, 0, 14), new THREE.Vector3(14, 0, -13)],
  [new THREE.Vector3(-11, 0, 12), new THREE.Vector3(0, 0, 0), new THREE.Vector3(11, 0, -11)],
  [new THREE.Vector3(11, 0, 11), new THREE.Vector3(0, 0, 0), new THREE.Vector3(-11, 0, -11)],
  [new THREE.Vector3(-6, 0, 8), new THREE.Vector3(6, 0, 8), new THREE.Vector3(6, 0, -6), new THREE.Vector3(-6, 0, -6)],
  [new THREE.Vector3(0, 0, 17), new THREE.Vector3(0, 0, 5)],
  [new THREE.Vector3(-5, 0, 17), new THREE.Vector3(-5, 0, 5)],
  [new THREE.Vector3(-10, 0, 0), new THREE.Vector3(10, 0, 0)],
];
const SHIRT_COLORS = [
  0x3a7bd5, 0xe74c3c, 0x2ecc71, 0xf39c12, 0x9b59b6,
  0x1abc9c, 0xe67e22, 0x34495e, 0xc0392b, 0x16a085,
  0x8e44ad, 0x2980b9, 0xd35400, 0x27ae60, 0xf5cba7,
  0xffffff, 0xffeb3b, 0x00bcd4, 0xff5722, 0x607d8b,
];
const PANTS_COLORS = [0x2c3e50, 0x34495e, 0x1a252f, 0x2e4057, 0x4a4a4a, 0x3d2b1f, 0x212121, 0x424242];
const SKIN_COLORS = [0xffcd94, 0xf0b27a, 0xe59866, 0xd4a574, 0xc68642, 0xa0522d];

// === v2: Bird constants ===
const BIRD_COUNT = 8;
const BIRD_FLY_HEIGHT_MIN = 9;
const BIRD_FLY_HEIGHT_MAX = 16;
const BIRD_RADIUS_MIN = 20;
const BIRD_RADIUS_MAX = 40;
const BIRD_SPEED_MIN = 0.20;
...
window.__threeScene = scene; // expose for boundary analysis
scene.background = new THREE.Color(0x86cfff);
scene.fog = new THREE.FogExp2(0xc8e8ff, 0.0022);
...
// === v2: Walking pedestrians and flying birds ===
const pedestriansGroup = createPedestrians();
scene.add(pedestriansGroup);
const birdsGroup = createBirds();
...
function loadModel(preset) {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      // v2: snap fence to actual model ground — model bottom is at bounds.min.y after offset
      if (fence) {
        const groundY = bounds.min.y;
        fence.position.y = groundY - 0.18; // embed 18cm into ground so no floating gap
      }
...
function setLighting(preset) {
  ...
  if (isNight) {
    ...
    // Subtle atmospheric haze for depth and realism (v2)
    scene.fog = new THREE.FogExp2(0xc8e8ff, 0.0022);
  }
}
...
// ============================================================
// PEDESTRIAN SYSTEM — v2
// Người đi bộ trong khuôn viên trường
// ============================================================

function createPedestrianMesh(shirtColor, pantsColor, skinColor) {
  const group = new THREE.Group();

  const skinMat = new THREE.MeshStandardMaterial({ color: skinColor, roughness: 0.85, metalness: 0 });
  const shirtMat = new THREE.MeshStandardMaterial({ color: shirtColor, roughness: 0.9, metalness: 0 });
  const pantsMat = new THREE.MeshStandardMaterial({ color: pantsColor, roughness: 0.85, metalness: 0 });
  const hairMat = new THREE.MeshStandardMaterial({ color: 0x1a0a00, roughness: 0.9, metalness: 0 });
  const shoeMat = new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.7, metalness: 0.1 });

  // Đầu
  const head = new THREE.Mesh(new THREE.SphereGeometry(0.056, 8, 6), skinMat);
  head.position.set(0, 0.445, 0);
  head.castShadow = true;
  group.add(head);

  // Tóc
  const hair = new THREE.Mesh(
    new THREE.SphereGeometry(0.059, 8, 5, 0, Math.PI * 2, 0, Math.PI * 0.52),
    hairMat,
  );
  hair.position.set(0, 0.458, 0);
  group.add(hair);

  // Cổ
  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.022, 0.022, 0.045, 6), skinMat);
  neck.position.set(0, 0.388, 0);
  group.add(neck);

  // Thân (áo)
  const torso = new THREE.Mesh(new THREE.CylinderGeometry(0.042, 0.045, 0.20, 7), shirtMat);
  torso.position.set(0, 0.28, 0);
  torso.castShadow = true;
  group.add(torso);

  // Cánh tay trái
  const leftArm = new THREE.Mesh(new THREE.CylinderGeometry(0.019, 0.019, 0.155, 5), shirtMat);
  leftArm.position.set(-0.068, 0.29, 0);
  leftArm.castShadow = true;
  group.add(leftArm);

  // Cánh tay phải
  const rightArm = new THREE.Mesh(new THREE.CylinderGeometry(0.019, 0.019, 0.155, 5), shirtMat);
  rightArm.position.set(0.068, 0.29, 0);
  rightArm.castShadow = true;
  group.add(rightArm);

  // Chân trái
  const leftLeg = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.023, 0.175, 6), pantsMat);
  leftLeg.position.set(-0.032, 0.09, 0);
  leftLeg.castShadow = true;
  group.add(leftLeg);

  // Chân phải
  const rightLeg = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.023, 0.175, 6), pantsMat);
  rightLeg.position.set(0.032, 0.09, 0);
  rightLeg.castShadow = true;
  group.add(rightLeg);

  // Bàn chân trái
  const leftFoot = new THREE.Mesh(new THREE.BoxGeometry(0.042, 0.022, 0.075), shoeMat);
  leftFoot.position.set(-0.032, 0.012, 0.016);
  group.add(leftFoot);

  // Bàn chân phải
  const rightFoot = new THREE.Mesh(new THREE.BoxGeometry(0.042, 0.022, 0.075), shoeMat);
  rightFoot.position.set(0.032, 0.012, 0.016);
  group.add(rightFoot);
...
function createPedestrians() {
  ...
  for (let i = 0; i < PEDESTRIAN_COUNT; i++) {
    const pathIndex = Math.floor(rand() * PEDESTRIAN_WALK_PATHS.length);
    const path = PEDESTRIAN_WALK_PATHS[pathIndex].map((p) => p.clone());
    const waypointIndex = Math.floor(rand() * path.length);

    // Vị trí ban đầu: nội suy ngẫu nhiên trên đoạn đường
    const startWp = path[waypointIndex];
    const nextWp = path[(waypointIndex + 1) % path.length];
    const t = rand();
    const startPos = new THREE.Vector3().lerpVectors(startWp, nextWp, t);

    const shirtColor = SHIRT_COLORS[Math.floor(rand() * SHIRT_COLORS.length)];
    const pantsColor = PANTS_COLORS[Math.floor(rand() * PANTS_COLORS.length)];
    const skinColor = SKIN_COLORS[Math.floor(rand() * SKIN_COLORS.length)];
    const speed = PEDESTRIAN_WALK_SPEED_MIN + rand() * (PEDESTRIAN_WALK_SPEED_MAX - PEDESTRIAN_WALK_SPEED_MIN);

    const mesh = createPedestrianMesh(shirtColor, pantsColor, skinColor);
    mesh.position.copy(startPos);
    mesh.position.y = MODEL_VERTICAL_OFFSET;

    mesh.userData.path = path;
    mesh.userData.waypointIndex = (waypointIndex + 1) % path.length;
    mesh.userData.speed = speed;
    mesh.userData.walkPhase = rand() * Math.PI * 2;
    mesh.userData.isPedestrian = true;
...
function updatePedestrians(delta) {
  pedestriansGroup.children.forEach((person) => {
    if (!person.userData.isPedestrian) return;

    const path = person.userData.path;
    const wpIdx = person.userData.waypointIndex;
    const target = path[wpIdx];
    ...
    if (dist < 0.08) {
      // Tới waypoint tiếp theo
      person.userData.waypointIndex = (wpIdx + 1) % path.length;
    } else {
      const moveSpeed = person.userData.speed * delta;
      const dir = diff.clone().normalize();
      person.position.x += dir.x * Math.min(moveSpeed, dist);
      person.position.z += dir.z * Math.min(moveSpeed, dist);
      person.position.y = MODEL_VERTICAL_OFFSET;

      // Quay mặt theo hướng di chuyển
      person.rotation.y = Math.atan2(dir.x, dir.z);

      // Animate chân tay
      person.userData.walkPhase += delta * person.userData.speed * 4.8;
      const swing = Math.sin(person.userData.walkPhase) * 0.52;

      const { leftArm, rightArm, leftLeg, rightLeg, leftFoot, rightFoot } = person.userData;

      if (leftArm) {
        leftArm.rotation.x = swing;
        leftArm.position.z = Math.sin(person.userData.walkPhase) * 0.022;
      }
      if (rightArm) {
        rightArm.rotation.x = -swing;
        rightArm.position.z = -Math.sin(person.userData.walkPhase) * 0.022;
      }
      if (leftLeg) {
        leftLeg.rotation.x = -swing * 0.85;
        leftLeg.position.z = Math.sin(person.userData.walkPhase) * 0.026;
      }
      if (rightLeg) {
        rightLeg.rotation.x = swing * 0.85;
        rightLeg.position.z = -Math.sin(person.userData.walkPhase) * 0.026;
      }
      if (leftFoot) {
        leftFoot.position.z = 0.016 + Math.sin(person.userData.walkPhase) * 0.026;
        leftFoot.position.y = 0.012 + Math.max(0, Math.sin(person.userData.walkPhase)) * 0.032;
      }
      if (rightFoot) {
        rightFoot.position.z = 0.016 - Math.sin(person.userData.walkPhase) * 0.026;
        rightFoot.position.y = 0.012 + Math.max(0, -Math.sin(person.userData.walkPhase)) * 0.032;
      }
    }
  });
}
...
// ============================================================
// BIRDS SYSTEM — v2
// Chim bay trên bầu trời khuôn viên trường
// ============================================================

function createBirdMesh() {
  const group = new THREE.Group();

  // Màu chim hơi tối, nhìn thấy trên nền trời
  ...
  leftWingGeo.setAttribute("position", new THREE.BufferAttribute(lverts, 3));
  ...
  rightWingGeo.setAttribute("position", new THREE.BufferAttribute(rverts, 3));
  ...
  const body = new THREE.Mesh(
    new THREE.CylinderGeometry(0.014, 0.022, 0.14, 5),
    wingMat.clone(),
  );
...
function createBirds() {
  ...
  const rand = createSeededRandom(98765);

  for (let i = 0; i < BIRD_COUNT; i++) {
    const bird = createBirdMesh();

    const radius = BIRD_RADIUS_MIN + rand() * (BIRD_RADIUS_MAX - BIRD_RADIUS_MIN);
    const height = BIRD_FLY_HEIGHT_MIN + rand() * (BIRD_FLY_HEIGHT_MAX - BIRD_FLY_HEIGHT_MIN);
    const angle = rand() * Math.PI * 2;
    const speed = BIRD_SPEED_MIN + rand() * (BIRD_SPEED_MAX - BIRD_SPEED_MIN);
    const flapPhase = rand() * Math.PI * 2;
    const flapSpeed = 2.8 + rand() * 2.4;
    const orbitDir = rand() > 0.5 ? 1 : -1; // Một số bay CW, một số CCW
    ...
    bird.userData.orbitRadius = radius;
    bird.userData.orbitHeight = height;
    bird.userData.orbitAngle = angle;
    bird.userData.orbitSpeed = speed * orbitDir;
    bird.userData.flapPhase = flapPhase;
    bird.userData.flapSpeed = flapSpeed;
    bird.userData.heightPhase = rand() * Math.PI * 2;
...
function updateBirds(delta) {
  ...
  birdsGroup.children.forEach((bird) => {
    if (!bird.userData.isBird) return;

    // Bay vòng quanh khuôn viên
    bird.userData.orbitAngle += bird.userData.orbitSpeed * delta;
    const angle = bird.userData.orbitAngle;
    const radius = bird.userData.orbitRadius;

    bird.position.x = Math.cos(angle) * radius + modelBounds.center.x;
    bird.position.z = Math.sin(angle) * radius + modelBounds.center.z;
    // Độ cao nhấp nhô nhẹ
    bird.position.y =
      bird.userData.orbitHeight +
      Math.sin(elapsed * 0.48 + bird.userData.heightPhase) * 1.4;

    // Quay mặt theo hướng bay
    const tangentX = -Math.sin(angle) * Math.sign(bird.userData.orbitSpeed);
    const tangentZ = Math.cos(angle) * Math.sign(bird.userData.orbitSpeed);
    bird.rotation.y = Math.atan2(tangentX, tangentZ);

    // Vỗ cánh
    bird.userData.flapPhase += bird.userData.flapSpeed * delta;
    const flapAngle = Math.sin(bird.userData.flapPhase) * 0.40;
...
// ============================================================
// FENCE SYSTEM — v2
// Hàng rào bao quanh khuôn viên trường
// ============================================================

function createFence() {
  const group = new THREE.Group();

  // Kích thước khuôn viên — hàng rào khít rìa bãi cỏ/mặt đường thực tế
  const HALF = 17.0;        // ground plane của model kết thúc ở ~±17, fence nằm trên cỏ
  const POST_HEIGHT = 2.2;  // trụ cao 2.2 đơn vị (~41% chiều cao tòa nhà 5.3)
  const POST_RADIUS = 0.14; // trụ dày, rõ từ xa
  const RAIL_HEIGHT = 0.10;
  const RAIL_DEPTH = 0.10;
  const PANEL_HEIGHT = 1.55; // tấm lưới cao
  const PANEL_THICK = 0.07;
  const POST_SPACING = 2.8;  // khoảng cách giữa các trụ
  const Y_BASE = MODEL_VERTICAL_OFFSET; // sẽ được điều chỉnh qua fence.position.y sau khi model load

  // Màu sắc
  const postMat = new THREE.MeshStandardMaterial({
    color: 0x2c3e2c,   // xanh lá đậm
    roughness: 0.5,
    metalness: 0.55,
  });
  const railMat = new THREE.MeshStandardMaterial({
    color: 0x253525,   // xanh đậm hơn
    roughness: 0.45,
    metalness: 0.6,
  });
  const panelMat = new THREE.MeshStandardMaterial({
    color: 0x2e5c3a,   // xanh lá cây đậm, tương phản bãi cỏ nhạt
    roughness: 0.6,
    metalness: 0.25,
    transparent: true,
    opacity: 0.92,
  ...
  const gateMat = new THREE.MeshStandardMaterial({
    color: 0xc8961e,   // vàng đồng
    roughness: 0.35,
    metalness: 0.8,
  });

  // Helper: tạo một đoạn rào dọc theo trục X hoặc Z
  // axis: 'x' hoặc 'z', fixed: giá trị cố định của trục còn lại
  function buildSide(axis, fixed, from, to, hasGate = false, gateCenter = 0, gateWidth = 5.6) {
    const length = Math.abs(to - from);
    const count = Math.ceil(length / POST_SPACING);
    const step = length / count;

    for (let i = 0; i <= count; i++) {
      const t = from + i * step;

      // Bỏ qua trụ trong vùng cổng
      if (hasGate && Math.abs(t - gateCenter) < gateWidth / 2 - 0.3) continue;

      const px = axis === 'x' ? t : fixed;
      const pz = axis === 'z' ? t : fixed;

      // Trụ
      const post = new THREE.Mesh(
        new THREE.CylinderGeometry(POST_RADIUS, POST_RADIUS * 1.15, POST_HEIGHT, 8),
        postMat,
      );
      post.position.set(px, Y_BASE + POST_HEIGHT / 2, pz);
      post.castShadow = true;
      group.add(post);

      // Mũ trụ
      const cap = new THREE.Mesh(
        new THREE.SphereGeometry(POST_RADIUS * 1.35, 6, 4),
        postMat,
      );
      cap.position.set(px, Y_BASE + POST_HEIGHT + POST_RADIUS * 1.35 * 0.5, pz);
      group.add(cap);
    }

    // Thanh ray trên và dưới
    const railLen = length;
    const railGeo = new THREE.BoxGeometry(
      axis === 'x' ? railLen : RAIL_DEPTH,
      RAIL_HEIGHT,
      axis === 'z' ? railLen : RAIL_DEPTH,
    );
    ...
    topRail.position.set(
      ...
      Y_BASE + POST_HEIGHT * 0.92,
      axis === 'z' ? (from + to) / 2 : fixed,
    );
    topRail.castShadow = true;
    group.add(topRail);

    // Rail chân — sát mặt đất, không có khoảng hở
    const bottomRail = topRail.clone();
    bottomRail.position.y = Y_BASE + RAIL_HEIGHT / 2;
    group.add(bottomRail);

    // Rail giữa
    const midRail = topRail.clone();
    midRail.position.y = Y_BASE + POST_HEIGHT * 0.50;
    group.add(midRail);

    // Tấm lưới (panels giữa các trụ) — bắt đầu từ mặt đất (Y_BASE) lên đến gần đỉnh trụ
    for (let i = 0; i < count; i++) {
      const tA = from + i * step;
      const tB = from + (i + 1) * step;
      const mid = (tA + tB) / 2;

      // Bỏ qua panel trong vùng cổng
      if (hasGate && Math.abs(mid - gateCenter) < gateWidth / 2) continue;

      const panelLen = step - 0.06;
      // Panel phủ từ Y_BASE (mặt đất) lên đến gần đỉnh trụ — không có khoảng hở bên dưới
      const fullPanelH = POST_HEIGHT * 0.88;
      const panelGeo = new THREE.BoxGeometry(
        axis === 'x' ? panelLen : PANEL_THICK,
        fullPanelH,
        axis === 'z' ? panelLen : PANEL_THICK,
      );
      const panel = new THREE.Mesh(panelGeo, panelMat);
      panel.position.set(
        axis === 'x' ? mid : fixed,
        Y_BASE + fullPanelH / 2,   // đáy panel tại Y_BASE = mặt đất
        axis === 'z' ? mid : fixed,
      );
  ...
  // Cổng vào chính (phía trước, z=+HALF, trung tâm x=0)
  function buildGate(x, z, axis) {
    const gateWidth = 5.4;
    const gateHeight = POST_HEIGHT * 1.15;
    const GATE_POST = POST_RADIUS * 2.2;

    // 2 trụ cổng
    [-gateWidth / 2, gateWidth / 2].forEach((offset) => {
      const gp = new THREE.Mesh(
        new THREE.CylinderGeometry(GATE_POST, GATE_POST * 1.2, gateHeight, 8),
        gateMat,
      );
      const gpx = axis === 'x' ? x + offset : x;
      const gpz = axis === 'z' ? z + offset : z;
      gp.position.set(gpx, Y_BASE + gateHeight / 2, gpz);
      gp.castShadow = true;
      group.add(gp);

      // Mũ trụ cổng — hình cầu vàng
      const gc = new THREE.Mesh(
        new THREE.SphereGeometry(GATE_POST * 1.6, 8, 6),
        new THREE.MeshStandardMaterial({ color: 0xd4a020, roughness: 0.3, metalness: 0.8 }),
      );
      gc.position.set(gpx, Y_BASE + gateHeight + GATE_POST * 1.4, gpz);
      group.add(gc);
    });

    // Thanh ngang trên cổng
    const archGeo = new THREE.BoxGeometry(
      axis === 'x' ? gateWidth : 0.06,
      0.06,
      axis === 'z' ? gateWidth : 0.06,
    );
    const arch = new THREE.Mesh(archGeo, gateMat);
    arch.position.set(x, Y_BASE + gateHeight, z);
    group.add(arch);

    // Cánh cổng (2 tấm)
    [-1, 1].forEach((side) => {
      const leafGeo = new THREE.BoxGeometry(
        axis === 'x' ? gateWidth / 2 - 0.12 : 0.045,
        gateHeight * 0.78,
        axis === 'z' ? gateWidth / 2 - 0.12 : 0.045,
      );
      const leaf = new THREE.Mesh(leafGeo, gateMat);
      const lx = axis === 'x' ? x + side * (gateWidth / 4) : x;
      const lz = axis === 'z' ? z + side * (gateWidth / 4) : z;
      leaf.position.set(lx, Y_BASE + gateHeight * 0.39, lz);
      group.add(leaf);
    });
  }

  // 4 cạnh rào
  // Phía trước (z = +HALF), cổng chính ở x=0
  buildSide('x', +HALF, -HALF, +HALF, true, 0, 5.6);
  // Phía sau  (z = -HALF)
  buildSide('x', -HALF, -HALF, +HALF, false);
  // Cạnh trái (x = -HALF)
  buildSide('z', -HALF, -HALF, +HALF, false);
  // Cạnh phải (x = +HALF)
  buildSide('z', +HALF, -HALF, +HALF, false);

  // Cổng vào chính phía trước
  buildGate(0, +HALF, 'x');
